...
impl GeneralizedProgram {
    ...
    pub fn matching_nodes<'a>(graph: &'a KernelStructureGraph, predicate: &dyn Predicate) -> Vec<&'a crate::structure::Node> {
        match predicate.evaluate(graph) {
            PredicateResult::RankedList(list) => list.into_iter()
                .filter_map(|(id, _)| graph.nodes.iter().find(|n| n.id == id))
                .collect(),
            PredicateResult::Bool(true) => graph.nodes.iter().collect(),
    ...
    fn apply_step(graph: &KernelStructureGraph, step: &AbstractStep, gw: u8, gh: u8) -> KernelStructureGraph {
        let candidates: Vec<crate::structure::Node> = match &step.condition {
            Some(pred) => Self::matching_nodes(graph, pred.as_ref()).into_iter().cloned().collect(),
