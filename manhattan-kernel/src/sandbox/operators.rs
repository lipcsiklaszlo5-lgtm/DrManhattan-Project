...
/// Transzformációs operátorok az ARC feladatokhoz
...
/// Transzformáció alkalmazása egy KSG gráfra (immutábilis, új gráfot ad vissza)
pub fn apply_transformation(graph: &KernelStructureGraph, transform: &Transformation) -> KernelStructureGraph {
    ...
    match transform {
        ...
        Transformation::Merge { node_a, node_b } => {
            // Két csomópont összeolvasztása: B törlése, A attribútumainak frissítése
        ...
        Transformation::Split { node_id } => {
            // Csomópont kettéválasztása: két kisebb objektumra bontás
            if let Some(node) = new_graph.nodes.iter().find(|n| &n.id == node_id).cloned() {
                ...
                if half_w > 0 {
                    ...
                    if let Some(new_node) = new_graph.nodes.last_mut() {
                        new_node.attributes = attrs1;
                    }
                    ...
                    if let Some(new_node) = new_graph.nodes.last_mut() {
                        new_node.attributes = attrs2;
                    }
                }
                
                // Eredeti csomópont törlése
                new_graph.nodes.retain(|n| &n.id != node_id);
                new_graph.edges.retain(|e| &e.from != node_id && &e.to != node_id);
            }
        }
...
/// Operátorsorozat szimulálása egy gráfon
